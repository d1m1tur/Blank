...
void Entity::createEntity(std::string name_, float x_, float y_, float scale_, int window_x, int window_y,
...
void Entity::moveTo(float x_, float y_)
{
    ...
    this->rect.x += x_; // move the x position with given amount
    this->rect.y += y_; // move the y position with given amount
}
